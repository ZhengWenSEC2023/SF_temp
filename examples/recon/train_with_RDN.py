...
import dataset_new as datasets
import model_RDN as models
import imageio
import time
import os
...
def train():
    ...
    for i in range(start_iter, args.num_iterations + 1):
        ...
        images_a, images_b, images_depth_a, images_depth_b, images_normal_a, images_normal_b, viewpoints_a, viewpoints_b = dataset_train.get_random_batch(args.batch_size)

        images_a = images_a.cuda()
        images_b = images_b.cuda()
        images_depth_a = images_depth_a.cuda()
        images_depth_b = images_depth_b.cuda()
        images_normal_a = images_normal_a.cuda()
        images_normal_b = images_normal_b.cuda()
        viewpoints_a = viewpoints_a.cuda()
        viewpoints_b = viewpoints_b.cuda()

        # soft render images
        render_images, rendered_depth, rendered_normal, laplacian_loss, flatten_loss = model([images_a, images_b],
                                                                                             [images_depth_a, images_depth_b],
                                                                                             [images_normal_a, images_normal_b],
        ...
        # compute loss
        normal_loss = multiview_mse_loss(rendered_depth, images_depth_a, images_depth_b)
        depth_loss = multiview_mse_loss(rendered_normal, images_normal_a, images_normal_b)
        ...
        loss = multiview_iou_loss(render_images, images_a, images_b) + \
               ...
               args.lambda_normal * normal_loss + \
               args.lambda_depth * depth_loss
        ...
        if i % args.demo_freq == 0:
            ...
            imageio.imsave(os.path.join(image_output, '%07d_fake.png' % i), img_cvt(render_images[0]))
