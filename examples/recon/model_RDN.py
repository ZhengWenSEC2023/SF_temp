...
class Model(nn.Module):
    ...
    def predict_multiview(self, image_a, image_b, image_depth_a, image_depth_b, image_normal_a, image_normal_b, viewpoint_a, viewpoint_b):
        ...
        res = self.renderer(vertices, faces)[0]
        silhouettes = res[0]
        depth = res[1]
        normal = res[2]
        return silhouettes.chunk(4, dim=0), depth.chunk(4, dim=0), normal.chunk(4, dim=0), laplacian_loss, flatten_loss
    ...
    def forward(self, images=None, images_depth=None, images_normal=None, viewpoints=None, voxels=None, task='train'):
        if task == 'train':
            return self.predict_multiview(images[0], images[1], images_depth[0], images_depth[1], images_normal[0], images_normal[1], viewpoints[0], viewpoints[1])
        elif task == 'test':
