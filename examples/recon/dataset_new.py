...
class ShapeNet(object):
    def __init__(self, directory=None, class_ids=None, set_name=None):
        ...
        for class_id in loop:
            images_depth.append(list(np.load(
                os.path.join(directory, '%s_depth_%s.npz' % (class_id, set_name))).items())[0][1])
            images_normal.append(list(np.load(
                os.path.join(directory, '%s_norma_%s.npz' % (class_id, set_name))).items())[0][1])
            images_rgba.append(list(np.load(
                os.path.join(directory, '%s_rgba_%s.npz' % (class_id, set_name))).items())[0][1])

            voxels.append(list(np.load(
                os.path.join(directory, '%s_voxel_%s.npz' % (class_id, set_name))).items())[0][1])
            self.num_data[class_id] = images_depth[-1].shape[0]
            self.pos[class_id] = count
            count += self.num_data[class_id]
        images_depth = np.concatenate(images_depth, axis=0).reshape((-1, 1, 64, 64))
        images_depth = np.ascontiguousarray(images_depth)

        images_normal = np.concatenate(images_normal, axis=0).reshape((-1, 3, 64, 64))
        images_normal = np.ascontiguousarray(images_normal)

        images_rgba = np.concatenate(images_rgba, axis=0).reshape((-1, 4, 64, 64))
        images_rgba = np.ascontiguousarray(images_rgba)


        self.images_depth = images_depth
        self.images_normal = images_normal
        self.images_rgba = images_rgba
    ...
    def get_random_batch(self, batch_size):
        ...
        images_depth_a = torch.from_numpy(self.images_depth[data_ids_a].astype('float32'))
        images_depth_b = torch.from_numpy(self.images_depth[data_ids_b].astype('float32'))
        images_a = torch.from_numpy(self.images_rgba[data_ids_a].astype('float32') / 255.)
        images_b = torch.from_numpy(self.images_rgba[data_ids_b].astype('float32') / 255.)
        images_normal_a = torch.from_numpy(self.images_normal[data_ids_a].astype('float32'))
        images_normal_b = torch.from_numpy(self.images_normal[data_ids_b].astype('float32'))
        ...
        return images_a, images_b, images_depth_a, images_depth_b, images_normal_a, images_normal_b, viewpoints_a, viewpoints_b
    ...
    def get_all_batches_for_evaluation(self, batch_size, class_id):
        ...
        for i in range((data_ids.size - 1) // batch_size + 1):
            images = torch.from_numpy(self.images_rgba[data_ids[i * batch_size:(i + 1) * batch_size]].astype('float32') / 255.)
            images_depth = torch.from_numpy(self.images_depth[data_ids[i * batch_size:(i + 1) * batch_size]].astype('float32'))
            images_normal = torch.from_numpy(self.images_normal[data_ids[i * batch_size:(i + 1) * batch_size]].astype('float32'))
