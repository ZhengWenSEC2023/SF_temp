...
def load_textures(filename_obj, filename_mtl, texture_res):
    ...
    # faces: [BS, # of faces, 3 (idx of faces), UV value]
    ...
    for material_name, filename_texture in list(texture_filenames.items()):
        ...
        # or has extral alpha channel should ignore for now
        if image.shape[2] == 4:
            image = image[:, :, :3]
...
def load_obj(filename_obj, normalization=False, load_texture=False, texture_res=4, texture_type='surface'):
    ...
    If input faces contains more than 3 vertices, the program will use triangles starts from a single vertices (0) to
        all other edges to represent the polygon.
    If texture_type == 'surface', call load_textures, load from mtl
    If texture_type == 'vertices', information of textures will be stored inside lines begin with 'v', as a RGB list
    If normalization: the model will be resized into a 1*1*1 cube
    About texture: index of three vertices and corresponding UV index are stored in 'f',
        a face could be decide by the index of vertices and location of vertices stored in 'v'
        the UV coordinates of texture could be map from (0, 1) to original size, stored in 'vt'
        sampled by bilinear interpolation, the texture of each face could be decided.
    """
